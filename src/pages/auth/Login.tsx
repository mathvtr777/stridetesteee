    ...
    return (
        <div className="relative flex min-h-screen w-full flex-col max-w-[430px] mx-auto overflow-x-hidden pb-10 bg-background-light dark:bg-background-dark font-display text-white selection:bg-primary/30">
            ...
            <div className="px-8 pt-6 pb-6">
                <h1 className="text-white text-[36px] font-extrabold leading-tight tracking-tight">Bem-vindo de volta</h1>
                <p className="text-white/60 text-lg font-medium leading-normal mt-2">Entre para continuar seu progresso.</p>
            </div>
            ...
            <div className="px-8 mt-10">
                ...
                >
                    <span className="text-white text-lg font-bold tracking-wide">{loading ? 'Entrando...' : 'Entrar'}</span>
                    {!loading && <Icon name="login" className="text-white text-[20px]" />}
                </button>
            </div>
            ...
            <div className="mt-auto pt-8 pb-4 text-center">
                <p className="text-white/40 font-medium text-sm">
                    Não tem uma conta?
                    <Link to="/auth/register" className="text-primary font-bold ml-1 hover:text-accent-purple transition-colors">Criar Conta</Link>
                </p>
            </div>
        </div>
    );
